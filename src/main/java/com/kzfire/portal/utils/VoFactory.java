...
public class VoFactory {
	...
	public static ConditionVo getConditionVo(HttpServletRequest request) {
		...
		Integer pages = 1;
		try {
			pages = Integer.valueOf(request.getParameter("pageNum") == null ? "1" : request.getParameter("pageNum"));
		} catch (Exception e) {
		}

		Integer pagesize = 20;
		try {
			pagesize = Integer.valueOf(request.getParameter("numPerPage") == null ? "20" : request.getParameter("numPerPage"));
		} catch (Exception e) {
		}
